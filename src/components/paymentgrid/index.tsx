...
const [selected, setSelected] = useState<("monthly" | "terms")[]>([]);
  ...
  const handleSubmit = () => {
    setSubmitted(true);
    setTimeout(() => setSubmitted(false), 3500); // Reset after 3.5 seconds
  };
 return (
...
<div className="hidden lg:inline-flex items-end gap-2 text-lg font-medium text-gray-500 hover:text-green-700 group transition-colors duration-300 cursor-pointer">
  See what i can fiannce with Gynger
  <ArrowRight
  ...
  />
        ...
        {/* Cards */}
 <div className="flex flex-col-reverse max-lg:pb-7 lg:flex-row justify-between gap-10 relative overflow-hidden rounded-3xl bg-[#f2f8f8] shadow-sm">
          {/* Background 'Pay' Text */}
          ...
          <div className="relative  lg:pl-13 lg:py-13 h-full px-5 md:px-12 z-10 w-full text-center lg:text-left">
            ...
            <p className="lg:mt-[40%] mt-7 text-xl text-[#0c4242] lg:max-w-md mx-auto md:mx-0">
              Extend flexible payment offers to your customers while getting paid up front.
            </p>
            <button className="mt-6 inline-flex max-lg:w-full items-center justify-between gap-2 bg-white text-[#007e7e] text-md font-medium px-5 py-4 rounded-lg shadow hover:bg-[#eef7f7] transition-all">
              <span className="2xl:bg-[#007e7e] max-2xl:bg-red-200 xl:bg-amber-200 text-white rounded-full px-2 py-1">
                💳
              </span>
              <span>
              Explore Gynger Pay
...
<div className="relative z-10 w-full flex justify-end">
  ...
  <div className="absolute md:space-y-6 bottom-0 right-0 w-[40%] sm:w-[200px] md:w-[280px] xl:w-[280px] bg-green-200 rounded-tl-2xl rounded-bl-2xl pl-5 sm:pl-7 md:pl-14 pt-5 sm:pt-7 md:pt-14 shadow-md z-0">
    ...
    <div className="mt-3 sm:mt-4 flex justify-between text-[0.6rem] sm:text-sm md:text-lg text-gray-700 pb-2">
      <div className="w-full">
        <div className="h-0.5 sm:h-1 md:h-1.5 w-full rounded-3xl bg-[#007e7e]" />
  ...
  <div className="relative mr-[35%] sm:mr-[30%] md:mr-[35%] lg:mr-[50%] md:min-w-[350px] xl:mr-[43%] 2xl:mr-[40%] my-[15%] sm:my-[16%] md:my-[17%] w-[180px] sm:w-[230px] md:w-[320px] lg:w-[360px] xl:w-[400px] bg-white/30 backdrop-blur-3xl rounded-xl sm:rounded-2xl shadow-xl p-3 sm:p-4 md:p-6 lg:p-9 z-10">
